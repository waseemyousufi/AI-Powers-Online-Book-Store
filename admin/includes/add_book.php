 <?php
    $title = $author = $price = $category = $rating = $publisher = $language = $description = "";
    ...
    if (isset($_POST["add_book"])) {
        ...
        if (array_filter($errors)) {
            $query = "INSERT INTO books (book_title, author_name, book_description, book_price, book_category, book_rating, publisher,book_image) VALUES('{$title}','{$author}','{$description}','{$price}','{$category}', '{$rating}','{$publisher}', '{$image}') ";
            $create_book_query = mysqli_query($conn, $query) or die(mysqli_error($conn));
        }
    }
                     ...
                     <select name="category" class="form-select" id="category">
                         <?php
                            ...
                            while ($row = mysqli_fetch_assoc($select_cat)) : ?>
                             <option value=""><?php echo $row["book_category"]; ?></option>
                         <?php endwhile; ?>
                     </select>
             ...
             <div class="col-lg-6">
                 <div class="mb-3">
                     ...
                     <div class="form-floating mt-4">
                         <textarea class="form-control fs-5" placeholder="Leave a comment here" id="floatingTextarea2" name="description" style="height: 150px"></textarea>
                         <label for="floatingTextarea2" class="fs-5">چکیده</label>
                     </div>

                 </div>
             </div>
             <button type="submit" name="add_book" class="btn-2 fs-5">افزودن به لست کتاب ها</button>
         </div>
     </form>
 </div>